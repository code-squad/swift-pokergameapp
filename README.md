...
`registerPlayer(of:)` 에서 dealer도 해당시키기 위해 `player` 속성을 만들었습니다. 이후 step에서 다른 메소드에도 쓰일 수 있을 것 같습니다. 또한 dealer 자신을 player에 넣은 것은 최대한 비슷하게 구성해보고자 했습니다. Dealer의 역할은 게임플레이와 동시에 PokerGame 진행을 맡고 있기 때문에 Player에 들어간다는걸 일부러 코드로 써서 표시해서 혼동이 없게금 하고 싶었습니다. 하지만 코드 line이 늘어나는 부분은 아쉽게 생각합니다.
